...
// Update the MapComponent to pass the onMarkerClick handler
const MapComponent = dynamic(() => import('@/components/Map').then(mod => {
  // Wrap the component to include the additional prop
  const EnhancedMap = (props: EnhancedMapProps) => {
...
export default function Home() {
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      // Don't close suggestions if we're in the selection process
      if (inSuggestionSelectionProcess.current) {
        return;
      }
    ...
    const fetchSuggestions = async () => {
      ...
      try {
        ...
        if (allGroups.length > 0) {
          ...
          // Only automatically show suggestions if the input is focused
  ...
  const handleMarkerClick = useCallback((groupId: string) => {
    console.log(`Marker clicked for group ${groupId}`);
    setSelectedGroupId(groupId);
    
    // Scroll to the selected group in the results list
    setTimeout(() => {
  ...
  // Update other handlers if necessary to use handleMarkerClick consistently
  const handleResultClick = useCallback((groupId: string) => {
    // Just delegate to our unified handleMarkerClick function
  ...
  const handleSelectSuggestion = (suggestion: SuggestionItem, event: React.MouseEvent) => {
    ...
    // CRITICAL: Force the dropdown to remain open
    ...
    // EXPLICITLY FORCE A NEW SEARCH REGARDLESS OF PREVIOUS TERM/TYPE
    // This ensures clicking ANY suggestion always triggers a search
    ...
    setTimeout(() => {
      ...
      // Clear the selection process flag after the search is complete
  ...
  const performSearch = async (
    ...
    // If it's the same search and we're not forcing it, we could skip
    // But for now, let's always proceed to ensure consistency
  ...
  // Add an effect to initialize the state from localStorage when on client side
  useEffect(() => {
    // Initialize from localStorage, but only on client side
    const storedValue = localStorage.getItem('useTestMap');
    setUseTestMap(storedValue === 'true');
  }, []);

  // Update the toggle function
  const toggleTestMap = () => {
    const newValue = !useTestMap;
    setUseTestMap(newValue);
    localStorage.setItem('useTestMap', newValue ? 'true' : 'false');
  };
              ...
              <div className="relative flex-grow flex items-center">
                ...
                <input
                  ...
                  onClick={(e) => {
                    ...
                    if (searchTerm.trim().length >= 2) {
                      setShowSuggestions(true); // Always show suggestions on click if we have a search term
                    }
                  }}
                  onFocus={() => {
                    if (searchTerm.trim().length >= 2 && suggestionItems.length > 0) {
                      setShowSuggestions(true);
                    }
                  }}
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {groups.map((group) => (
                          <div
                            ...
                            onClick={() => handleMarkerClick(group.id)}
                          >
                            <div className="flex flex-col">
                              <div>
        ...
        <div className="lg:w-[70%] h-full overflow-hidden">
          ...
          >
            <div className="flex justify-between items-center mb-3">
              ...
              {/* Add a temporary debug button to toggle test map */}
              <button 
                onClick={toggleTestMap}
                className="px-3 py-1 bg-gray-100 hover:bg-gray-200 text-xs rounded border"
              >
                {useTestMap ? 'Switch to Regular Map' : 'Try Test Map'}
              </button>
            </div>
            ...
            <div className="flex-grow relative" style={{ minHeight: "400px" }}>
              {isLoadingInitial ? (
                ...
                <>
                  {/* Conditionally render either the regular map or the test map */}
                  {useTestMap ? (
                    <TestMap height="100%" />
                  ) : (
                    <MapComponent 
